#!/usr/bin/env node
// Sepolia demo: Dfns master key, WDK account derived from it, transfer signed by Dfns, confirmation.
// Usage: wdk-dfns-demo [--fresh-key] [--to 0x...] [--amount 0.0001] [--dry-run]
...
const { values: opts } = parseArgs({ options: {
  'fresh-key': { type: 'boolean', default: false },
  to: { type: 'string' },
...
const network = process.env.DFNS_NETWORK || 'EthereumSepolia'
const client = clientFromEnv()

console.log('WDK wallet on Sepolia, signing with Dfns.')

step('Dfns master key')
let masterKeyId = opts['fresh-key'] ? undefined : process.env.DFNS_MASTER_KEY_ID
if (masterKeyId) {
  out('key', `${masterKeyId} (from DFNS_MASTER_KEY_ID)`)
} else {
  ({ id: masterKeyId } = await client.keys.createKey({ body: { scheme: 'ECDSA', curve: 'secp256k1', masterKey: true, name: `wdk-demo ${new Date().toISOString().slice(0, 16)}` } }))
  out('key', `${masterKeyId} (created, no wallets yet)`)
}

step('WDK wallet manager backed by DfnsSignerEvm')
const wallet = new WalletManagerEvm(new DfnsSignerEvm({ client, masterKeyId, network }), {
...
step('Message signature through Dfns')
...
if (opts['dry-run']) {
  ...
  out('signed by', Transaction.from(signed).from === address ? 'Dfns, not broadcast (--dry-run)' : 'MISMATCH')
  wallet.dispose()
  process.exit(0)
}
...
console.log(`\nDone. Dfns master key ${masterKeyId}, account ${address}, tx ${hash}`)
wallet.dispose()
